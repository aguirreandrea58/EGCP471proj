let isValidCommand,parseStringWithIndefiniteNumberOfParams,getValueForClosestKey,getClosestElemWithKey,getTargetElemsForKeyName,getValueForKeyName,forEachAttr,showWarning,optionsData;_756‍.x([["callWatchFunctionsOnElements",()=>callWatchFunctionsOnElements],["triggerWatchAttributes",()=>triggerWatchAttributes],["executeCommandOnMultipleElements",()=>executeCommandOnMultipleElements],["getTargetsForCommand",()=>getTargetsForCommand]]);_756‍.w("../common/get-valid-element-properties",[["isValidCommand",["isValidCommand"],function(v){isValidCommand=v}]]);_756‍.w("../parse-data-attributes",[["parseStringWithIndefiniteNumberOfParams",["parseStringWithIndefiniteNumberOfParams"],function(v){parseStringWithIndefiniteNumberOfParams=v}]]);_756‍.w("./getAndSetKeyValues",[["getValueForClosestKey",["getValueForClosestKey"],function(v){getValueForClosestKey=v}],["getClosestElemWithKey",["getClosestElemWithKey"],function(v){getClosestElemWithKey=v}],["getTargetElemsForKeyName",["getTargetElemsForKeyName"],function(v){getTargetElemsForKeyName=v}],["getValueForKeyName",["getValueForKeyName"],function(v){getValueForKeyName=v}]]);_756‍.w("../hummingbird/lib/dom",[["forEachAttr",["forEachAttr"],function(v){forEachAttr=v}]]);_756‍.w("../common/show-error",[["showWarning",["showWarning"],function(v){showWarning=v}]]);_756‍.w("../inputjs/optionsData",[["default",["optionsData"],function(v){optionsData=v}]]);
...
// e.g. callWatchFunctionsOnElements(document.querySelectorAll("[watch]"))
  ...
  elems.forEach(elem => {
    _756‍.a("forEachAttr",forEachAttr)(elem, (attrName, attrValue) => {
      if (attrName.indexOf("watch:") === 0) {
        let dashCaseKeyName = attrName.substring("watch:".length);
        let closestElemWithKey = _756‍.a("getClosestElemWithKey",getClosestElemWithKey)({ elem, keyName: dashCaseKeyName });
        let value = _756‍.a("getValueForKeyName",getValueForKeyName)({ elem: closestElemWithKey, keyName: dashCaseKeyName });
        let targetElems = _756‍.a("getTargetElemsForKeyName",getTargetElemsForKeyName)({ elem, keyName: dashCaseKeyName });
...
// note: this allows for watching multiple keys with the same name as long as they're at different levels
  ...
  watchElems.forEach(watchElem => {
    ...
    let listOfCommands = _756‍.a("parseStringWithIndefiniteNumberOfParams",parseStringWithIndefiniteNumberOfParams)(watchAttrValue);
    listOfCommands.forEach(({ funcName, args }) => {
      if (_756‍.a("isValidCommand",isValidCommand)({ commandName: funcName })) {
        executeCommand({ elem: watchElem, commandName: funcName, value, method: "set" });
      } else {
        // call custom watch function
        let watchFunc = _756‍.a("optionsData",optionsData).watchFunctions && _756‍.a("optionsData",optionsData).watchFunctions[funcName];
...
//     native prop: get/set a property on the current element
  ...
  for (let i = 0; i < targetElems.length; i++) {
    ...
    if (_756‍.a("isValidCommand",isValidCommand)({ commandName })) {
      if (method === "set") {
...
function executeCommand({ elem, commandName, value, method }) {
  if (commandName.indexOf("@attr:") === 0) {
    ...
    if (prop.toLowerCase() === "src" && elem.tagName.toLowerCase() === "img") {
      _756‍.a("showWarning",showWarning)(
